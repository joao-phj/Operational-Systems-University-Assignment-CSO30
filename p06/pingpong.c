...
void task_yield () {
  ...
  #endif
  prev = current;
  current->processing_time += systime() - start_task;
...
void task_exit (int exitCode) {
  ...
  if(current->tid != 1) {
    ...
    current->processing_time += systime() - start_task;
    current->overall_time = systime() - current->start_time;

    printf("Task %d: execution time %u ms, processor time %u ms, %d activations", current->tid, current->overall_time, current->processing_time, current->activations);
